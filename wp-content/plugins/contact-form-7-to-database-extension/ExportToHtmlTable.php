...
class ExportToHtmlTable extends ExportBase implements CFDBExport {
    ...
    public function export($formName, $options = null) {
        ...
        <table <?php if ($this->htmlTableId) echo "id=\"$this->htmlTableId\" "; if ($this->htmlTableClass) echo "class=\"$this->htmlTableClass\"" ?> >
            ...
            <?php if ($canDelete) { ?>
            <th id="delete_th">
                <button id="delete" name="cfdbdel" class="button" onclick="this.form.submit()"><?php echo htmlspecialchars(__('Delete', 'contact-form-7-to-database-extension'))?></button>
            ...
            foreach ($this->dataIterator->getDisplayColumns() as $aCol) {
                $colDisplayValue = htmlentities($aCol, null, 'UTF-8'); // no HTML injection
                if ($this->headers && isset($this->headers[$aCol])) {
                    $colDisplayValue = htmlentities($this->headers[$aCol], null, 'UTF-8');;
                }
